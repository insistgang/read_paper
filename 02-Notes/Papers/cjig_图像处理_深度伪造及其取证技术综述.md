---
title: "cjig 图像处理 深度伪造及其取证技术综述"
pdf: "[[cjig_图像处理_深度伪造及其取证技术综述.pdf]]"
primary_topic: "I_场景文本_OCR"
topics: [I_场景文本_OCR, J_人脸_表情_伪造_情感, R_综述_Survey]
status: unread
reading_stage: filename-only
confidence: low
source_basis: filename
date_added: 2026-06-03
tags: [paper]
---

# cjig 图像处理 深度伪造及其取证技术综述

## 基本信息
- **PDF**: [[cjig_图像处理_深度伪造及其取证技术综述.pdf]]
- **primary_topic**: `I_场景文本_OCR`
- **topics**: `I_场景文本_OCR`, `J_人脸_表情_伪造_情感`, `R_综述_Survey`
- **状态**: unread (仅根据文件名推断, 待阅读全文确认)
- **date_added**: 2026-06-03

## 文件名推断

> 仅根据文件名推断, 未读 PDF 内容。读完 PDF 后请把 `source_basis` 改为 `pdf-text`,
> 并把 `confidence` / `reading_stage` 升级。

**提取的关键词**:

- cjig
- 图像处理
- 深度伪造及其取证技术综述

**文件名模式**: `cjig_图像处理_深度伪造及其取证技术综述`

## 可能研究方向

> 基于 primary_topic + topics 推测, 仅作入门指引。

- 属于主题: **I_场景文本_OCR**
- 关联主题: J_人脸_表情_伪造_情感, R_综述_Survey

## 待读问题

> 读 PDF 前先列 2-3 个你想在论文里找到答案的问题。

1.
2.
3.

## 阅读笔记

> 边读边记; 用 `[[双链]]` 关联到其他论文笔记 / 主题首页。

## 关键贡献

> 读完填

## 方法

> 读完填

## 实验

> 读完填

## 局限

> 读完填

## 相关论文

> 用 `[[]]` 链接到其他论文笔记或主题首页, 例如 [[I_场景文本_OCR]]。

