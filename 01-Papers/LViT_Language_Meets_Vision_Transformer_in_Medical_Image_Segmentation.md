---
title: "LViT Language Meets Vision Transformer in Medical Image Segmentation"
pdf: "[[LViT_Language_Meets_Vision_Transformer_in_Medical_Image_Segmentation.pdf]]"
primary_topic: "A1_医学图像_分割"
topics: [A1_医学图像_分割, F_图像分割_语义_实例_全景]
status: unread
reading_stage: filename-only
confidence: low
source_basis: filename
date_added: 2026-06-03
tags: [paper]
---

# LViT Language Meets Vision Transformer in Medical Image Segmentation

## 基本信息
- **PDF**: [[LViT_Language_Meets_Vision_Transformer_in_Medical_Image_Segmentation.pdf]]
- **primary_topic**: `A1_医学图像_分割`
- **topics**: `A1_医学图像_分割`, `F_图像分割_语义_实例_全景`
- **状态**: unread (仅根据文件名推断, 待阅读全文确认)
- **date_added**: 2026-06-03

## 文件名推断

> 仅根据文件名推断, 未读 PDF 内容。读完 PDF 后请把 `source_basis` 改为 `pdf-text`,
> 并把 `confidence` / `reading_stage` 升级。

**提取的关键词**:

- LViT
- Language
- Meets

**文件名模式**: `LViT_Language_Meets_Vision_Transformer_in_Medical_Image_Segmentation`

## 可能研究方向

> 基于 primary_topic + topics 推测, 仅作入门指引。

- 属于主题: **A1_医学图像_分割**
- 关联主题: F_图像分割_语义_实例_全景

## 待读问题

> 读 PDF 前先列 2-3 个你想在论文里找到答案的问题。

1.
2.
3.

## 阅读笔记

> 边读边记; 用 `[[双链]]` 关联到其他论文笔记 / 主题首页。

## 关键贡献

> 读完填

## 方法

> 读完填

## 实验

> 读完填

## 局限

> 读完填

## 相关论文

> 用 `[[]]` 链接到其他论文笔记或主题首页, 例如 [[A1_医学图像_分割]]。

